 ...
 * this function simply returns 6, so that the caller can add it to the
 * count_map. This function links box-120 with box-29 by setting sequence[120]
 * = 29. if max_count = 5 and no box with size-5 can fit inside the current box,
 * we decrement it to 4 and try again,
 * if there is a match, we return 5, because try=4, and fit, so it must be 4
 * + 1. if there is no match, we decrement until we reach try=1 if still no
 * match, we return 1, since 1 is the smallest sequence. a box that can't fit
 * any other box has a sequence of 1. [55]
 *
...
/*
 * The function creates a count_map. then for each row index, 0-k, adds them to
 * the count_map. count_map with a value of {1 => {0, 1}, 2 => {2, 5}, 3 => {3,
 * 4}, 4 => {6, 7} } would indicate that box-0 and box-1 is the biggest box in a
 * sequence of 1 box. and and box-3 and box-4 is the biggest box in a sequence
 * of 3 boxes. it returns the max_count, in this case, 4. it also modifies the
 * parameter passed by ref, for track. k - the number of rows n - the number of
 * columns source - the 2 dimentional matrix sequence - we need this to
 * backtrack. eg: [-1, -1, 1, 2, 2, 1, 4, 4] the 0-th box is too small to
 * contain anything the 1-st box is too small to contain anything the 2-nd box
 * contains box-1 (totaling 2, including self) the 3-rd box contains box-2
 * (totaling 3, including self) the 4-th box contains box-2 (totaling 3,
 * including self) the 5-th box contains box-1 (totaling 2, including self) the
 * 6-th box contains box-4 (totaling 4, including self) the 7-th box contains
 * box-4 (totaling 4, including self) the largest number is 4, so the max
 * seqeunce is 4 4 can be found at index 6 or 7 so we can pick either 6 or 7
 *            element-6 has the value 4
 *            add that to our array [6, 4]
 *            element-4 has the value 2
 *            add that to our array [6, 4, 2]
 ...
 *         since count_map[max_count] is a vector, any value in the vector is
 * valid. to make it easy, we choose index-0. returns the max_count, we need
 * this to display the longest sequence
 */
...
void populate_stack(stack<int> &st, int track, int *sequence,
                    map<int, int> &position_map) {
  while (track != -1) {
    // cout << "track = " << track << endl;
    st.push(position_map[track] + 1);
    // cout << track << ": ";
...
void print_sequence(stack<int> &st) {
  string sep = "";
  while (!st.empty()) {
    cout << sep << st.top();
    st.pop();
 ...
 * this is needed to tranlate the after-sorted position to a before-sorted
 * position
 *
 */
void process(int k, int n) {
  ...
  for (int i = 0; i < k; i++) {
    int *p = new int[n];
    source[i] = p;
    box_position_map[p] = i;
  }

  read(k, n, source);

  for (int i = 0; i < k; i++) {
    position_map[i] = box_position_map[source[i]];
  }
  ...
  stack<int> st;
  cout << max_count << endl;
  populate_stack(st, track, sequence, position_map);
  print_sequence(st);
